...
export default function Home() {
  ...
  async function loadNFTs() {
      ...
      //Now we will start interacting with our Smart-Contracts.

      const data = await marketContract.fetchMarketItems();

      const items = await Promise.all(data.map(async i => {                                       // We are mapping over our 'data' 
    ...
    <div className="flex justify-center">
      <div className="px-4" style={{ maxWidth: "1600px" }}>
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 pt-4">
          ...
          {nfts.map((nft, i) => (      
                                                                   // Mapping over the 'nfts' i.e our state.
...
5] Note : You can use one RPC to deploy contract on testnet and other RPC to interact with it. Its completely fine
   Eg :- Contract deployment on Mumbai testnet :- Public RPC.
         Contract Interaction :- Moralis Speedy Node RPC.
